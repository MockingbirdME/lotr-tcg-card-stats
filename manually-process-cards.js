  ...
  for (const game of games[`${format}`]) {
    ...
    for (const deck of game.decks) {
      ...
      for (const cardId of deck.adventureDeck) {
        if (!cardStats.cards[cardId]) cardStats.cards[cardId] = {
          type: "site",
          format: {}
        }
  
        if (!cardStats.cards[cardId].format[format]) {
          cardStats.cards[cardId].format[format] = {
            deckCount: 0,
            wins: 0,
            losses: 0
          }        
        ...
        cardStats.cards[cardId].format[format].deckCount++;

        if (win) cardStats.cards[cardId].format[format].wins++;
        else cardStats.cards[cardId].format[format].losses++
      ...
      for (const cardId in deck.drawDeck) {
        ...
        if (!cardStats.cards[cardId].format[format]) {
          ...
          console.log(cardStats.cards[cardId]);
          cardStats[`${format}Cards`].uniqueDrawDeckCards++
        }
  
        cardStats.cards[cardId].format[format].deckCount++;
        cardStats.cards[cardId].format[format].cardCount+= deck.drawDeck[cardId].inDeck;
...
for (const [id, card] of Object.entries(cardStats.cards)) {
  ...
  csvRows.push(
    `${id}, ${card.type},${card.format.fellowship?.winRate || ""},${card.format.fellowship?.deckCount || ""},${card.format.fellowship?.rating || "N/A"},${card.format.movie?.winRate || ""},${card.format.movie?.deckCount || ""},${card.format.movie?.rating || "N/A"},${card.format.expanded?.winRate || ""},${card.format.expanded?.deckCount || ""},${card.format.expanded?.rating || "N/A"}`)
