...
export default class Card {
  ...
  static getCardId(cardNumber) {
    const nonPromoId = promoMap[cardNumber] || cardNumber;

    const [ set, card ] = nonPromoId.split("_");

    if (!set || !card) console.log(`No set or card for ${cardNumber}`);

    if (set >= 50 && set < 79) return getCardName(`${set - 50}_${card}`);

    return getCardName(`${set}_${card}`);
  }
  ...
  static async loadAllNumericIds() {
      const view = 'allCardsByNumericId';
